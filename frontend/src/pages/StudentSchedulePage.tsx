      ...
      <Row className="g-4 mb-4">
          ...
          <div className="ms-stat-card lavender">
            ...
            <div className="ms-stat-content">
              <div className="ms-stat-value">
                {new Set(assignments.map((a) => a.location_name)).size}
              </div>
        ...
        <Col xs={6} lg={3}>
          <div className="ms-stat-card coral">
            ...
            <div className="ms-stat-content">
              <div className="ms-stat-value">
                {days.filter((d) => getAssignmentsForDay(d).length > 0).length}
              </div>
          ...
          {days.length === 0 ? (
            <div className="ms-empty-state" style={{ margin: '40px' }}>
              ...
              <p className="ms-empty-description">
                Time slots haven't been configured yet. Contact an administrator to set up the
                schedule.
              </p>
            </div>
          ) : (
            <Row className="g-0">
              {[0, 1, 2, 3, 4, 5, 6].map((dayOfWeek) => {
                ...
                return (
                  <Col
                    key={dayOfWeek}
                    style={{ borderRight: dayOfWeek < 6 ? '1px solid var(--ms-border)' : 'none' }}
                  >
                    {/* Day Header */}
                    <div
                      ...
                      {dayAssignments.length === 0 ? (
                        ...
                        <div className="d-flex flex-column" style={{ gap: '16px' }}>
                          {dayAssignments.map((assignment) => {
                            const ts = timeSlots.find((t) => t.id === assignment.time_slot_id);
                            const locColorIndex = getLocationColorIndex(
