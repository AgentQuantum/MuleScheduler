...
function AdminSchedulePage() {
  ...
  const { data, loading, error, refreshData, usersById, locationsById, timeSlotsById } = useScheduleData({
    weekStart,
    locationFilter,
    userFilter
  })
  ...
  const handleAssignmentMove = async (
    ...
    try {
      ...
      const errorMsg = err.response?.data?.message || err.response?.data?.error || 'Failed to move shift'
    ...
    <div className="ms-animate-in">
      ...
      <Row className="g-4 mb-4">
        ...
        <ShiftDetailsDrawer
          ...
          users={data.users}
          locations={data.locations}
          timeSlot={selectedAssignment ? timeSlotsById.get(selectedAssignment.time_slot_id) : undefined}
          isOpen={isDrawerOpen}
          onClose={() => {
            setIsDrawerOpen(false)
            setSelectedAssignment(null)
