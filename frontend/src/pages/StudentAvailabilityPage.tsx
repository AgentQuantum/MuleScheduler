...
function StudentAvailabilityPage() {
  ...
  const getSlotForDayAndTime = (day: number, startTime: string, endTime: string): TimeSlot | undefined => {
    const existingSlots = timeSlots.filter(ts => ts.day_of_week === day)
    const daySlots = existingSlots.length > 0 ? existingSlots : generateDefaultSlots(day)
  ...
  const availableCount = Array.from(availabilities.values())
    .filter(a => a.preference_level === 1 && a.location_id === selectedLocationId).length
  const preferredCount = Array.from(availabilities.values())
    .filter(a => a.preference_level === 2 && a.location_id === selectedLocationId).length
            ...
            <div className="ms-card mb-4">
              <div className="ms-card-body" style={{ padding: '12px 20px' }}>
                <div className="d-flex align-items-center gap-3 flex-wrap">
                  <span style={{ fontSize: '0.85rem', color: 'var(--ms-text-muted)', fontWeight: 600 }}>
                  ...
                  {locations.map((loc, idx) => (
                    <button
                      ...
                      style={{
                        ...
                        border: selectedLocationId === loc.id ? '2px solid var(--ms-primary)' : '2px solid transparent'
                      }}
                    >
                      {loc.name}
                    </button>
                ...
                <thead>
                  <tr>
                    ...
                    {ALL_DAYS.map((day, idx) => (
                      <th key={day} className="day-header">{DAY_NAMES[idx]}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                      ...
                      {ALL_DAYS.map(day => {
                        ...
                        return (
                          <td 
                            ...
                            title={hasSlot ? (status === 0 ? 'Click to mark available' : status === 1 ? 'Click to mark preferred' : 'Click to clear') : 'Not scheduled'}
                          >
        ...
        <div className="availability-save-bar">
          <div className="availability-save-content">
            <div className="availability-save-info">
              <strong>{availableCount + preferredCount}</strong> slots selected 
