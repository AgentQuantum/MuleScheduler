...
interface WeeklyScheduleGridProps {
  ...
  timeSlots: TimeSlot[]
  weekStart: string
  onAssignmentMove: (assignmentId: number, target: { userId: number; locationId: number; timeSlotId: number }) => Promise<void>
...
const SortableShiftCard: React.FC<SortableShiftCardProps> = ({ assignment, timeSlot, onClick }) => {
  const {
    ...
    transition,
    isDragging
  } = useSortable({ id: `assignment-${assignment.id}` })
...
const WeeklyScheduleGrid: React.FC<WeeklyScheduleGridProps> = ({
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const targetTimeSlot = timeSlots.find(ts => ts.day_of_week === targetDay) || 
      ...
      <div className="schedule-grid">
        ...
        <Row className="g-0">
          <Col xs="auto" className="schedule-grid-row-header" style={{ minWidth: '200px' }}>
            <div style={{ padding: '18px 20px', fontWeight: 700, fontSize: '0.9rem', color: 'var(--ms-text-primary)' }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="me-2" style={{ opacity: 0.6 }}>
