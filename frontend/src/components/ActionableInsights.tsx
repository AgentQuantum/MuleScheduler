...
const ActionableInsights: React.FC<ActionableInsightsProps> = ({
  ...
  const workersWithShifts = new Set(assignments.filter(a => a.user_id != null).map(a => String(a.user_id)))
  ...
  if (idleWorkers.length > 0) {
    insights.push({
      ...
      description: idleWorkers.map(w => w.name.split(' ')[0]).join(', ') + ' ha' + (idleWorkers.length > 1 ? 've' : 's') + ' no shifts',
    ...
    <div className={`actionable-insights ${isExpanded ? 'expanded' : 'collapsed'}`}>
      ...
      >
        <div className="insights-title">
          ...
          {actionableCount > 0 && (
            <span className="insights-badge warning">{actionableCount}</span>
          )}
        </div>
