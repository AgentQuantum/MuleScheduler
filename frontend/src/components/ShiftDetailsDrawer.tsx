...
interface ShiftDetailsDrawerProps {
  ...
  isOpen: boolean
  onClose: () => void
  onUpdate: (assignmentId: number, updates: { userId?: number; locationId?: number; timeSlotId?: number }) => Promise<void>
...
const ShiftDetailsDrawer: React.FC<ShiftDetailsDrawerProps> = ({
  ...
  const hasChanges = assignment && (
    selectedUserId !== assignment.user_id ||
    selectedLocationId !== assignment.location_id
  )
  ...
  return (
    <>
      {/* Backdrop */}
      <div 
        className={`ms-drawer-backdrop ${isOpen ? 'open' : ''}`}
        onClick={onClose}
      />
      ...
      <div className={`ms-drawer ${isOpen ? 'open' : ''}`}>
        <div className="ms-drawer-header">
          <h5 className="ms-drawer-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <div className="ms-drawer-body">
          {assignment ? (
            <>
              {/* Time & Day Info Card */}
              <div className="ms-card mb-4" style={{ 
                ...
                <div className="ms-card-body" style={{ padding: '20px' }}>
                  <div className="d-flex align-items-center gap-3">
                    ...
                    <div>
                      <div style={{ fontWeight: 700, color: 'var(--ms-text-primary)', fontSize: '1.1rem' }}>
                        {timeSlot ? `${formatTime(timeSlot.start_time)} – ${formatTime(timeSlot.end_time)}` : 'N/A'}
                      </div>
                      <div style={{ color: 'var(--ms-text-secondary)', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Icons.Calendar size={14} /> {timeSlot ? getDayName(timeSlot.day_of_week) : 'N/A'}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mb-4 p-4" style={{ background: 'var(--ms-surface)', borderRadius: '14px' }}>
                <div className="d-flex align-items-center justify-content-between mb-3">
                  <h6 className="ms-label mb-0" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <div className="d-flex align-items-center gap-3">
                  {assignment.user_name ? (
                    <>
                      ...
                      />
                      <div>
                        <div style={{ fontWeight: 600, fontSize: '1rem', color: 'var(--ms-text-primary)' }}>
                          {assignment.user_name}
                        </div>
                        <span className={`ms-location-badge loc-${locColorIndex}`} style={{ marginTop: '4px', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                    ...
                    <div className="d-flex align-items-center gap-2">
                      ...
                      <div>
                        <div style={{ fontWeight: 600, color: 'var(--ms-text-muted)' }}>No worker assigned</div>
                        <span className={`ms-location-badge loc-${locColorIndex}`} style={{ marginTop: '4px', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                          <Icons.MapPin /> {assignment.location_name}
                        </span>
                      </div>
            ...
            <div className="ms-empty-state">
              ...
              <p className="ms-empty-description">Click on a shift card to view and edit its details.</p>
            </div>
          )}
        </div>
