  ...
  return (
    ...
    >
      <div className="d-flex justify-content-between align-items-start mb-2">
        <span className={`ms-location-badge loc-${locColorIndex}`} style={{ fontSize: '0.7rem', padding: '4px 10px', display: 'flex', alignItems: 'center', gap: '4px' }}>
      ...
      <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--ms-text-primary)', display: 'flex', alignItems: 'center', gap: '6px' }}>
        <Icons.Calendar /> {getDayName(timeSlot.day_of_week)} • {formatTime(timeSlot.start_time)} – {formatTime(timeSlot.end_time)}
      </div>
      <div style={{ marginTop: '8px' }}>
        <div className="ms-intensity-bar">
          <div 
            className={`ms-intensity-fill ${remaining >= 3 ? 'high' : remaining >= 2 ? 'medium' : 'low'}`}
            style={{ width: `${Math.min((shift.currentWorkers / shift.requiredWorkers) * 100, 100)}%` }}
          />
        </div>
        <span style={{ fontSize: '0.75rem', color: 'var(--ms-text-muted)', marginTop: '4px', display: 'block' }}>
  ...
  return (
    <div className="ms-unassigned-panel">
      <div className="ms-unassigned-header">
        <h5 className="ms-unassigned-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <div className="ms-unassigned-body">
        {shifts.length === 0 ? (
          <div className="ms-unassigned-empty">
            ...
            <p style={{ fontSize: '0.9rem', fontWeight: 500, marginBottom: '4px', color: 'var(--ms-text-secondary)' }}>
              All shifts assigned!
            </p>
