...
const DraggableWorker: React.FC<{ user: User; color: typeof WORKER_COLORS[0]; isDragging?: boolean }> = ({ user, color, isDragging }) => {
  ...
  return (
    ...
    >
      ...
      <div className="worker-details">
        <div className="worker-name" style={{ color: color.text }}>{user.name}</div>
  ...
  return (
    ...
    >
      <div className="shift-block-content">
        <UserAvatar name={user?.name || '?'} userId={assignment.user_id} size="xs" showBorder={false} />
    ...
    if (activeData?.type === 'worker') {
      ...
      if (overData?.type === 'slot') {
        ...
        const existing = assignments.find(
          a => a.user_id === userId && a.time_slot_id === overData.timeSlotId && a.location_id === overData.locationId
        )
        if (!existing) {
    ...
    >
                  ...
                  <div key={timeRange} className="calendar-time-row">
                    ...
                    {ALL_DAYS.map(day => {
                      ...
                      const slotForDay = timeSlots.find(
                        ts => ts.day_of_week === day && ts.start_time === start && ts.end_time === end
                      )
      ...
      <DragOverlay>
        {activeWorker && (
          ...
          >
            <UserAvatar name={activeWorker.name} userId={activeWorker.id} size="sm" showBorder={false} />
