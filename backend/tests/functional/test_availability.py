...
from models import Location, TimeSlot, User, UserAvailability


@pytest.fixture
def test_availability_setup(test_app, test_user, test_location, test_time_slot):
    """Set up availability test data."""
    return {"user": test_user, "location": test_location, "time_slot": test_time_slot}
...
class TestGetAvailability:
    ...
    def test_get_availability_authenticated(self, client, auth_token):
        ...
        response = client.get(
            "/api/availability", headers={"Authorization": f"Bearer {auth_token}"}
        )
    ...
    def test_get_availability_returns_user_entries_only(
        self, test_app, test_availability_setup, auth_token
    ):
...
class TestCreateAvailability:
    ...
    def test_create_availability_updates_existing(
        self, client, auth_token, test_availability_setup
    ):
    ...
    def test_create_availability_default_preference(
        self, client, auth_token, test_availability_setup
    ):
...
class TestCreateAvailabilityBatch:
    ...
    def test_batch_create_unauthorized(self, client, test_availability_setup):
        ...
        response = client.post(
            "/api/availability/batch", json={"week_start_date": "2024-05-01", "entries": []}
        )
        assert response.status_code == 401
    ...
    def test_batch_empty_entries(self, client, auth_token):
        ...
        response = client.post(
            "/api/availability/batch",
            json={"week_start_date": "2024-07-01", "entries": []},
            headers={"Authorization": f"Bearer {auth_token}"},
        )
        assert response.status_code == 201
