...
class TestGetSettings:
    ...
    def test_get_settings_as_admin(self, client, admin_token):
        ...
        response = client.get("/api/settings", headers={"Authorization": f"Bearer {admin_token}"})
    ...
    def test_get_settings_as_user_forbidden(self, client, auth_token):
        ...
        response = client.get("/api/settings", headers={"Authorization": f"Bearer {auth_token}"})
...
class TestUpdateSettings:
    ...
    def test_update_both_settings(self, client, admin_token):
        ...
        response = client.put(
            "/api/settings",
            json={"max_workers_per_shift": 4, "max_hours_per_user_per_week": 15},
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 200
        data = response.get_json()
    ...
    def test_update_settings_unauthorized(self, client):
        """Unauthenticated request returns 403."""
        response = client.put("/api/settings", json={"max_workers_per_shift": 10})
        assert response.status_code == 403
