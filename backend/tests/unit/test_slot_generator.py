...
class TestDeleteSlotsForDay:
    ...
    def test_delete_slots_for_day(self, test_app):
        ...
        with test_app.app_context():
            ...
            for i in range(3):
                slot = TimeSlot(day_of_week=3, start_time=time(9 + i, 0), end_time=time(10 + i, 0))
                db.session.add(slot)
            db.session.commit()
...
class TestRegenerateSlotsForDay:
    ...
    def test_regenerate_slots_replaces_old(self, test_app):
        ...
        with test_app.app_context():
            ...
            old_slot = TimeSlot(day_of_week=4, start_time=time(8, 0), end_time=time(9, 0))
            ...
            old = TimeSlot.query.filter_by(day_of_week=4, start_time=time(8, 0)).first()
            assert old is None
...
class TestRegenerateAllSlots:
    ...
    def test_regenerate_all_clears_existing(self, test_app):
        ...
        with test_app.app_context():
            ...
            manual = TimeSlot(day_of_week=6, start_time=time(8, 0), end_time=time(9, 0))
...
class TestCountSlotsForDay:
    ...
    def test_count_slots_for_day(self, test_app):
        ...
        with test_app.app_context():
            # Create slots
            for i in range(5):
                slot = TimeSlot(day_of_week=0, start_time=time(9 + i, 0), end_time=time(10 + i, 0))
                db.session.add(slot)
            db.session.commit()
