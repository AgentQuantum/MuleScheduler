...
class TestCalculateHours:
    ...
    def test_calculate_hours_partial_shift(self, test_app):
        ...
        with test_app.app_context():
            time_slot = TimeSlot(day_of_week=0, start_time=time(10, 0), end_time=time(14, 0))
            hours = calculate_hours(time_slot)
            assert hours == 4.0
    ...
    def test_calculate_hours_overnight_shift(self, test_app):
        ...
        with test_app.app_context():
            time_slot = TimeSlot(
                day_of_week=0, start_time=time(22, 0), end_time=time(6, 0)  # 10 PM  # 6 AM next day
            )
            hours = calculate_hours(time_slot)
            assert hours == 8.0
    ...
    def test_calculate_hours_30_minute_shift(self, test_app):
        ...
        with test_app.app_context():
            time_slot = TimeSlot(day_of_week=0, start_time=time(9, 0), end_time=time(9, 30))
            hours = calculate_hours(time_slot)
            assert hours == 0.5
...
class TestGetUserTotalHours:
    ...
    def test_get_user_total_hours_single_assignment(
        self, test_app, test_user, test_location, test_time_slot
    ):
...
class TestHasOverlappingAssignment:
    ...
    def test_has_overlapping_assignment_no_overlap(
        self, test_app, test_user, test_location, test_time_slot
    ):
    ...
    def test_has_overlapping_assignment_with_overlap(
        self, test_app, test_user, test_location, test_time_slot
    ):
...
class TestRunAutoScheduler:
    ...
    def test_run_auto_scheduler_prefers_preferred_slots(
        self, test_app, test_location, test_time_slot
    ):
    ...
    def test_run_auto_scheduler_uses_shift_requirement_override(
        self, test_app, test_location, test_time_slot
    ):
    ...
    def test_run_auto_scheduler_skips_already_assigned(
        self, test_app, test_location, test_time_slot
    ):
    ...
    def test_run_auto_scheduler_respects_blocked_slots(
        self, test_app, test_location, test_time_slot
    ):
    ...
    def test_run_auto_scheduler_creates_default_settings(
        self, test_app, test_location, test_time_slot
    ):
    ...
    def test_run_auto_scheduler_skips_slots_with_no_availabilities(
        self, test_app, test_location, test_time_slot
    ):
    ...
    def test_run_auto_scheduler_handles_race_condition_overlap(
        self, test_app, test_location, test_time_slot
    ):
