...
def get_weekly_overrides():
    ...
    overrides = WeeklyScheduleOverride.query.filter_by(week_start_date=week_start_date).order_by(WeeklyScheduleOverride.day_of_week).all()
...
def update_weekly_override(override_id):
    ...
    if 'start_time' in data:
        ...
        override.start_time = time.fromisoformat(start_time_str) if ':' in start_time_str else time.fromisoformat(start_time_str + ':00')
    if 'end_time' in data:
        end_time_str = data['end_time']
        override.end_time = time.fromisoformat(end_time_str) if ':' in end_time_str else time.fromisoformat(end_time_str + ':00')
...
def create_override_from_standard():
    ...
    standard_schedules = DaySchedule.query.filter_by(is_active=True).order_by(DaySchedule.day_of_week).all()
