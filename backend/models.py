...
class UserAvailability(db.Model):
    ...
    time_slot_id = db.Column(db.Integer, db.ForeignKey('time_slots.id'), nullable=False)
    week_start_date = db.Column(db.Date, nullable=False)
    preference_level = db.Column(db.Integer, default=1, nullable=False)  # 1 = neutral, 2 = preferred
    ...
    __table_args__ = (db.UniqueConstraint('user_id', 'location_id', 'time_slot_id', 'week_start_date', name='unique_availability'),)
    ...
    def to_dict(self):
        ...
        if self.time_slot and self.week_start_date:
            ...
            event_date = datetime.combine(
                ...
                datetime.min.time()
            ) + timedelta(days=self.time_slot.day_of_week)
