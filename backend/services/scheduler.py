...
def cleanup_orphaned_records():
    ...
    orphaned_assigns = Assignment.query.filter(
        ~Assignment.time_slot_id.in_(valid_slot_ids)
    ).all()
    ...
    if orphaned_avails or orphaned_assigns:
        db.session.commit()
        print(f"Cleaned up {len(orphaned_avails)} orphaned availabilities and {len(orphaned_assigns)} orphaned assignments")
...
def run_auto_scheduler(week_start_date):
    ...
    for location in locations:
        for time_slot in time_slots:
            ...
            max_capacity = override_map.get(
                (location.id, time_slot.id), 
                default_max
            )
            ...
            current_count = get_current_assignment_count(
                time_slot.id, location.id, week_start_date
            )
